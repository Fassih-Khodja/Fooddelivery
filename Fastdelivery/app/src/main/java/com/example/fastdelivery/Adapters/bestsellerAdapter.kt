...
class bestsellerAdapter(var l:List<Food>?, private val listner:bestselleronitemclicklistner): RecyclerView.Adapter<bestsellerviewholder>() { // this is the class of adapter
    ...
    override fun onBindViewHolder(holder: bestsellerviewholder, position: Int) {
        ...
        holder.image.transitionName = "image_${position+100}" // il faux changer le faix de +100 , il faux de trouver une autre method
        holder.itemView.setOnClickListener {

                l?.get(position)?.let { it1 -> listner.bestselleronitemclick(position+100, it1,holder.image) }
